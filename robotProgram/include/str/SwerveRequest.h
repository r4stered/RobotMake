...
public:
  ...
  virtual ctre::phoenix::StatusCode Apply(
    ...
    std::array<SwerveModule, 4>& modules) override
  {
    for (size_t i = 0; i < modules.size(); i++) {
      frc::SwerveModuleState state{
        0_mps, parameters.swervePositions[i].Angle()};
...
public:
    ...
    std::array<SwerveModule, 4>& modules) override
  {
    ...
    if (units::math::sqrt(toApplyX * toApplyX + toApplyY * toApplyY)
      < deadband) {
      toApplyX = 0_mps;
      toApplyY = 0_mps;
    }
...
public:
    ...
    std::array<SwerveModule, 4>& modules) override
  {
    ...
    units::radians_per_second_t toApplyOmega
      = units::radians_per_second_t{headingController.Calculate(
        ...
        targetDirection.Radians().value(), parameters.timestamp)};
    if (units::math::sqrt(toApplyX * toApplyX + toApplyY * toApplyY)
      < deadband) {
      toApplyX = 0_mps;
      toApplyY = 0_mps;
  ...
  FieldCentricFacingAngle& withRotationalDeadband(
    units::radians_per_second_t rotDeadband)
  {
    this->rotationalDeadband = rotDeadband;
    return *this;
  }
